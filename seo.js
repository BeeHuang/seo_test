...
module.exports = {
  SeoCheck : function (){
    ...
    this.rule4 = function rule4() {
      ...
      this.output +='--- Start 4 Detect if there’re more than '+strongLimitCount+' <strong> tag in HTML ('+strongLimitCount+' is a value should be configurable by user) \n';
      if($('strong').length < strongLimitCount) {
        this.output +='error: there’re more than '+strongLimitCount+' <strong> tag in HTML\n';
      }
      this.output +='--- End 4 Detect if there’re more than '+strongLimitCount+' <strong> tag in HTML ('+strongLimitCount+' is a value should be configurable by user) \n';
    };
    ...
    this.getOutput = function() {
      if (this.outputMethod == 1) {
        ...
        var tmpFile = 'tmpSeoOutput.txt';
        fs.writeFile(tmpFile, this.output, function(err) {
          if(err) {
            throw Error('create file error='+err);
            return ;
          }
